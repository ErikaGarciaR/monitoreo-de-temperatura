interface interface_monitoreo (
  ...
  task automatic enviar_temperatura(input logic signed [10:0] valor);
    ...
    @(negedge clk);  // Bajar la bandera después del flanco de subida
    sensor_valid = 1'b0;  // 
  endtask
  //
  task automatic reporte_estado();  //Muestra en consola el estado completo del sistema,
    $display(
        "[INTERFACE]  %t | cont_bajo:%0d | cont_alto:%0d | Alerta: %b | Vent: %b | Cal: %b | Estado: %b",
