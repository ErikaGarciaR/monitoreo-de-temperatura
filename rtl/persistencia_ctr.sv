...
module persistencia_ctr #(
    ...
    input  logic       sensor_valid,  // verificar si hay lectura nueva
  ...
  always_ff @(posedge clk, negedge arst_n) begin
    if (!arst_n) begin
      ...
      if (es_bajo) begin
        if (sensor_valid && (cont_bajo < N)) cont_bajo <= cont_bajo + 1;
      ...
      if (es_alto) begin
        if (sensor_valid && (cont_alto < N)) cont_alto <= cont_alto + 1;
